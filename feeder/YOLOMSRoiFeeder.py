...
class YOLOMSRoiFeeder(data.Dataset):
    def __init__(self, roidb, ratio_list, ratio_index, num_classes, training=True, normalize=None,
                 **args):
        self._roidb = roidb
        ...
        # define initial size index
        self.input_size = args['mix_args']['input_size']
        self.size_index = 0
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            # The condition to change size:
            # (1) this index is the beginning of a batch; (2) accu_batch_num % batch_per_size == 0
            if index % self.batch_size == 0:
                if self.accu_batch_num % self.args['train_feeder_args']['batch_per_size'] == 0:
                    self.size_index = np.random.randint(0, 10)
                    self.accu_batch_num = 0
                self.accu_batch_num += 1


        # accumulate the number of batch: the first index of each batch can be mod by batchsize
        ...
        data = torch.from_numpy(blobs['data'])  # 1 * h * w * 3
        im_info = torch.from_numpy(blobs['im_info'])  # 1 * 3
        data_height, data_width = data.size(1), data.size(2)

        if self.training:
            np.random.shuffle(blobs['gt_boxes'])
            gt_boxes = torch.from_numpy(blobs['gt_boxes'])  # N * 5
